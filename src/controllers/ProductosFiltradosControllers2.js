...
const filtrarEdad = (edad, array1) =>{
	if (edad){
		const arr = []
		array1.map((a) => { if(a.edad === edad){ arr.push(a)}})
		return arr	
	}else{
		return array1
	}
...
const filtrarGenero = (genero, array1) =>{
	if (genero){
		const arr = []
		array1.map((a) => { if(a.genero === genero){ arr.push(a)}})
		return arr	
	}else {
		return array1
	}
...
const filtrarTalla = async (talla, array1) =>{
	if(talla){
		const arr = []
		array1.map((a) => { if(a.stock.hasOwnProperty(talla)){if(a.stock[talla].length){ arr.push(a)}}})
		return arr	
	}else{
		return array1
	}
...
const filtrarColor = (color,talla, array1) =>{
	if (color){
		const arr = []
		if(talla){
		array1.map((a) => { if(a.stock[talla].find(item => item.color === color)){arr.push(a)}})
		}else{
			array1.map((a) => {if (Object.values(a.stock).some(tallas =>
				tallas.find(item => item.color === color))){arr.push(a)}
			})
		}
		return arr	
	}else{
...
const filtrarPrecio = (minPrecio, maxPrecio, array1) =>{
	if (minPrecio && maxPrecio){
		const arr = []
		array1.map((a) => { if(a.precio < maxPrecio && a.precio > minPrecio){ arr.push(a)}})
		return arr	
	}else{
		return array1
	}
